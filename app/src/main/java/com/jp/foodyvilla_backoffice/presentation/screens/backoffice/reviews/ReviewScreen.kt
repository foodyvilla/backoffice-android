...
fun ReviewScreen(
    ...
    val reviews = remember(state.rows, state.searchQuery) {
        state.rows.map { it.toModel<Review>() }.filter {
        ...
        item {
            PremiumCard {
                Row(
                    ...
                    if (session?.canCreate("reviews") == true) {
                        Button(
                            onClick = onCreate,
        ...
        if (state.isLoading) {
            ...
            item {
                EmptyState(
                    ...
                    message = "Wait for customers to share their feedback.",
