...
class OutletManagementViewModel(
    ...
    fun uploadMenuImagesList(context: Context, uris: List<Uri>) {
        viewModelScope.launch {
            _state.update { it.copy(isLoading = true) }
            runCatching {
                repository.uploadMultipleImages(context, uris, "menu-items", "menu")
            }.onSuccess { uploadedUrls ->
                _state.update { it.copy(mMenuImagesList = it.mMenuImagesList + uploadedUrls, isLoading = false) }
            }.onFailure { err ->
    ...
    fun uploadOutletLogo(context: Context, uri: Uri) {
        viewModelScope.launch {
            _state.update { it.copy(isLoading = true) }
            runCatching {
                repository.uploadSingleImage(context, uri, "outlet-logos", "logos")
            }.onSuccess { url ->
    ...
    fun uploadOutletBanner(context: Context, uri: Uri) {
        viewModelScope.launch {
            _state.update { it.copy(isLoading = true) }
            runCatching {
                repository.uploadSingleImage(context, uri, "outlet-logos", "banners")
            }.onSuccess { url ->
    ...
    fun clearDishImagesArray() { _state.update { it.copy(mMenuImagesList = emptyList()) } }

    // --- INITIALIZERS FOR TYPE-SAFE NAVIGATION WORKSPACES ---
    fun setupOutletFormWorkspace(editId: Long?) {
        viewModelScope.launch {
            if (editId == null) {
                _state.update { it.copy(targetOutletId = null, oName = "", oAddress = "", oCity = "", oPhone = "", oEmail = "", oRadius = "5.0", oLat = "21.1983", oLng = "81.9614", oOpensAt = "09:00:00", oClosesAt = "23:00:00", oRazorPayKey = "rzp_test_ShBw7mlCM6gT6y", oUploadedLogoUrl = null, oUploadedBannerUrl = null) }
            } else {
                _state.update { it.copy(isLoading = true) }
                runCatching { repository.fetchOutletById(editId) }.onSuccess { o ->
                    _state.update { it.copy(targetOutletId = o.id, oName = o.name, oAddress = o.address, oCity = o.city, oPhone = o.phone, oEmail = o.email, oRadius = o.radiusKm.toString(), oLat = o.lat.toString(), oLng = o.lng.toString(), oOpensAt = o.opensAt ?: "09:00:00", oClosesAt = o.closesAt ?: "23:00:00", oRazorPayKey = o.razorPayKey, oUploadedLogoUrl = o.logoUrl, oUploadedBannerUrl = o.bannerUrl, isLoading = false) }
                }
            }
        }
    }
    ...
    fun setupMenuFormWorkspace(editId: Long?, activeOutletId: Long) {
        viewModelScope.launch {
            ...
            val catalog = catalogRepository.getAllCatalogProducts()
            if (editId == null) {
                _state.update { it.copy(globalProductCatalogOptions = catalog, targetMenuItemId = null, mSelectedProduct = null, mPrice = "", mDiscount = "0", mHandlingCharges = "0.0", mDeliveryCharges = "0.0", mIsFreeDelivery = false, mIsAvailable = true, mIsOutOfStock = false, mMenuImagesList = emptyList(), isLoading = false) }
            } else {
                runCatching { repository.fetchMenuItemById(editId) }.onSuccess { m ->
                    val prodMatch = catalog.find { it.id == m.productId }
                    _state.update { it.copy(globalProductCatalogOptions = catalog, targetMenuItemId = m.id, mSelectedProduct = prodMatch, mPrice = m.price.toString(), mDiscount = m.discount.toString(), mHandlingCharges = m.handlingCharges.toString(), mDeliveryCharges = m.deliveryCharges.toString(), mIsFreeDelivery = m.isFreeDelivery, mIsAvailable = m.isAvailable, mIsOutOfStock = m.isOutOfStock, mMenuImagesList = m.imagesList, isLoading = false) }
                }
            }
        }
    }
    ...
    fun onOLatChanged(v: String) { _state.update { it.copy(oLat = v) } }
    fun onOLngChanged(v: String) { _state.update { it.copy(oLng = v) } }
    fun onOOpensAtChanged(v: String) { _state.update { it.copy(oOpensAt = v) } }
    fun onOClosesAtChanged(v: String) { _state.update { it.copy(oClosesAt = v) } }
    fun onORazorPayKeyChanged(v: String) { _state.update { it.copy(oRazorPayKey = v) } }
    ...
    fun onMHandlingChargesChanged(v: String) { _state.update { it.copy(mHandlingCharges = v) } }
    fun onMDeliveryChargesChanged(v: String) { _state.update { it.copy(mDeliveryCharges = v) } }
    fun onMFreeDeliveryToggled(v: Boolean) { _state.update { it.copy(mIsFreeDelivery = v) } }
    ...
    fun commitOutletAction(onSuccess: () -> Unit) {
        ...
        val payload = OutletUiModel(id = s.targetOutletId ?: 0L, name = s.oName, address = s.oAddress, city = s.oCity, phone = s.oPhone, email = s.oEmail, logoUrl = s.oUploadedLogoUrl, bannerUrl = s.oUploadedBannerUrl, lat = s.oLat.toDoubleOrNull() ?: 21.1983, lng = s.oLng.toDoubleOrNull() ?: 81.9614, radiusKm = s.oRadius.toDoubleOrNull() ?: 5.0, opensAt = s.oOpensAt, closesAt = s.oClosesAt, razorPayKey = s.oRazorPayKey)

        viewModelScope.launch {
            _state.update { it.copy(isLoading = true) }
            runCatching { if (s.targetOutletId == null) repository.insertNewOutlet(payload) else repository.updateOutletRow(payload) }
                .onSuccess { onSuccess() }.onFailure { err -> emitError(err.localizedMessage ?: "Failed to save branch parameters.") }
        }
    }

    fun commitOutletMenuAction(activeOutletId: Long, onSuccess: () -> Unit) {
        val s = _state.value
        val prodId = s.mSelectedProduct?.id ?: return emitError("Linking a valid product catalog template is required.")
        val payload = OutletMenuItemUiModel(id = s.targetMenuItemId ?: 0L, outletId = activeOutletId, productId = prodId, imagesList = s.mMenuImagesList, price = s.mPrice.toDoubleOrNull() ?: 0.0, discount = s.mDiscount.toLongOrNull() ?: 0L, isAvailable = s.mIsAvailable, isOutOfStock = s.mIsOutOfStock, handlingCharges = s.mHandlingCharges.toDoubleOrNull() ?: 0.0, deliveryCharges = s.mDeliveryCharges.toDoubleOrNull() ?: 0.0, isFreeDelivery = s.mIsFreeDelivery)

        viewModelScope.launch {
            _state.update { it.copy(isLoading = true) }
            runCatching { if (s.targetMenuItemId == null) repository.addProductToOutletMenu(payload) else repository.updateOutletMenuRow(payload) }
                .onSuccess { onSuccess() }.onFailure { err -> emitError(err.localizedMessage ?: "Failed to save menu changes.") }
        }
    }
